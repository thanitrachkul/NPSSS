...
const DataEntry: React.FC<DataEntryProps> = ({ onAddStudents, studyPlans, subjects }) => {
  ...
  const [residence, setResidence] = useState<'IN_DISTRICT' | 'OUT_DISTRICT' | ''>(''); // New State
  ...
  const handleDownloadTemplate = async () => {
    ...
    const headers = [
        ...
        'Residence (ในเขต/นอกเขต)', // New Column
    ...
    // Example data
    const exampleRow = ['STU001', 'เด็กชาย', 'ตัวอย่าง', 'รักเรียน', 'ในเขต', ...studyPlans.map(p => p.name), ...subjects.map(() => 80)];
